    ...
    $.widget('magenest.giftcard', {
        ...
        _binAddButton:function(config) {
            ...
            addBtn.click(function() {
                ...
                var template = jQuery( this.options.tableId).find('.sample-template').html();

                var trElements = jQuery( this.options.tableId).find('tbody').find('tr');
                ...
                var valueFind = '/value=\".+\"/';
                templateRow = this._replaceAll(templateRow, valueFind, ' ');

                var find = "[0]";
                var re = new RegExp(find, 'g');

                var replace = "[" + next_id + "]";
                var res = this._replaceAll(templateRow, find, replace);

                find = '_0_';
                replace = '_' + next_id + '_';
                re = new RegExp(find, 'g');
                res = this._replaceAll(res, find, replace);
                var newRow =  res + is_new_template + '</tr>';



                jQuery( this.options.tableId).find('tbody').append(newRow);
            });
        }
