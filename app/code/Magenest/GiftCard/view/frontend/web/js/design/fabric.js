	...
	return function(options) {
		...
		self.addImage = function(imageURL) {
			fabric.Image.fromURL(imageURL, function(object) {
                ...
                //add a scale
                object.set({scaleY:0.2,scaleX:0.2});
        ...
        /**
         * change the background of the gift card
         */
        self.setBGImage = function (urlBG) {
           // console.log('change background');
            self.setBackgroundImage(urlBG) ;
            self.render();
        } //end of change the background image

        self.addArt = function(urlArt) {
            self.addImage(urlArt);
            self.render();
        }
        ...
        self.addBackground = function () {
            self.selectedObject.type ="changebackground";

        }
        //Add Art to the Gift Card
        //=============================================================
        self.addArt = function() {
          //  self.uploadImageActive = true;
           // alert('set it to true');

            self.selectedObject.type ='uploadimage';
        }
        //add by Luu Thanh Thuy


        //
        //submitDesign to the server
        //=============================================================
        ////
        self.submitDesign = function(name) {
            self.deactivateAll();

            //  var blobCanvas  = self.getCanvasBlob();
            var strDataURI = canvas.toDataURL();
            strDataURI = strDataURI.substr(22, strDataURI.length);
            $.ajax({
                url : 'http://127.0.0.1/magento2.1/giftcard/design/save',
                type: 'POST',
                data :{image: strDataURI}
            });
